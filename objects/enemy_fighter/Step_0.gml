...
if instance_exists(friendly_fighter){
	...
	if distance_to_object(friendly_fighter) >= 1000 {
		if speed > 0 {
			speed -= .25;
		}
	}
	if distance_to_object(friendly_fighter) <= 70 {
		image_angle -= 7;
		direction -= 3;
		motion_add(image_angle, .1);
	}
	...
	if distance_to_object(friendly_fighter) <= 750 {
