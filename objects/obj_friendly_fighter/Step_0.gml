...
if instance_exists(obj_enemy_fighter){
	ex = instance_nearest(x, y, obj_enemy_fighter).x;
	ey = instance_nearest(x, y, obj_enemy_fighter).y;

	if distance_to_object(obj_enemy_fighter) >= 150 {
...
if distance_to_object(obj_enemy_fighter) >= 1000 {
	if speed > 0 {
		speed -= .25;
	}
}
if distance_to_object(obj_enemy_fighter) <= 70 {
	image_angle -= 7;
	direction -= 3;
	motion_add(image_angle, .1);
}
...
if distance_to_object(obj_enemy_fighter) <= 750 {
...
if instance_exists(obj_space_enemy_bfl){
	ex = instance_nearest(x, y, obj_space_enemy_bfl).x;
	ey = instance_nearest(x, y, obj_space_enemy_bfl).y;

	if distance_to_object(obj_space_enemy_bfl) >= 150 {
		directionVar = point_direction(x, y, ex, ey);
			if direction >= directionVar {
				image_angle -= 4;
				direction -= 1;
...
if distance_to_object(obj_space_enemy_bfl) >= 1000 {
...
if distance_to_object(obj_space_enemy_bfl) <= 750 {
